...
public class MyBatisTest {
	public static void main(String[] args) {
		...
		try {
			...
			sqlSession = sessionFactory.openSession(true);	//오토 커밋 여부 설정
		...
		// 2) SqlSession객체변수를 이용하여 해당 쿼리문을 실행
		// 형식) sqlSession.insert("namespace값.id값",파라미터객체);
		// 		반환값 : 성공한 레코드 수
		
		cnt = sqlSession.insert("memberTest.insertMember", mv);
		...
		*/
		...
		/*
		// 1) 응답결과가 여러개일 경우...
		System.out.println("select 연습 시작(결과가 여러개)");
		...
		*/
		...
		// 2) 응답결과가 1개일 경우...
		System.out.println("select 연습 시작(결과가 1개)");
		
		// 응답결과가 1개가 확실할 경우에는 selectOne()메서드를 사용
