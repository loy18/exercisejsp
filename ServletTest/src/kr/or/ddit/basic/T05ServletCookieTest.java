...
/*
	...
	2. 동작방식
		...
		- 쿠키 저장단계: 웹브라우저는 응답데이터에 포함된 쿠키를 쿠키저장소에 보관
		- 쿠키 전송단계: 웹브라우저는 저장한 쿠키를 요청이 있을때마다 웹서버에 전송
						(삭제전까지) 웹서버는 브라우저가 전송한 쿠키를 이용해 작업수행
*/
	@Override
...
//		readCookieExam(req,resp);
	...
	/*
		...
		1. 쿠키객체 생성하기. 사용불가문자(공백,[]()=,"/?@:;)
			...
			쿠키값은 사용불가문자를 제외한 나머지 출력가능한 아스키 문자 사용가능.
				=> 이외의 값(예: 한글)을 사용시에는 URLEncoder.encode()를 사용하여 처리
		
		2. 쿠기 최대 지속시간 설정(초단위) => 지정하지 않으면 브라우저 종료시 삭제
