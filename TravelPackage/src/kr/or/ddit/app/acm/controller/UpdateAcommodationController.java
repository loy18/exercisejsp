...
public class UpdateAcommodationController extends HttpServlet {
   ...
   protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String acmCont = req.getParameter("acmCont");
		...
		// 첨부파일 저장하기
		AtchFileVO atchFileVO = fileService.saveAtchFileList(req, getServletContext().getRealPath("/") + File.separator + "acm");
	   ...
	   if(atchFileVO == null) { // 새로운 첨부파일이 존재하지 않으면
		   // 기존 첨부 파일 ID 설정하기
		   acmv.setAtchFileId(Long.parseLong(atchFileId));
	   } else { 
		   // 신규 첨부 파일 ID 설정하기
		   acmv.setAtchFileId(atchFileVO.getAtchFileId());
	   }
